...
&$PYTHON_CMD src/train.py --model mobilenet --epochs $EPOCHS --batch_size $BATCH --optimizer adam --lr 0.0001

Write-Host "`n>>> [4/8] Fine-Tuning: SGD (LR=0.001)" -ForegroundColor Cyan
&$PYTHON_CMD src/train.py --model mobilenet --epochs $EPOCHS --batch_size $BATCH --optimizer sgd --lr 0.001
...
&$PYTHON_CMD src/train.py --model mobilenet --epochs $EPOCHS --batch_size $BATCH --optimizer adam --lr 0.0001 --weight_decay 0.0001 --use_cutout

Write-Host "`n>>> [8/8] Fine-Tuning + Weight Decay + Cutout (SGD, LR=0.001)" -ForegroundColor Cyan
&$PYTHON_CMD src/train.py --model mobilenet --epochs $EPOCHS --batch_size $BATCH --optimizer sgd --lr 0.001 --weight_decay 0.0001 --use_cutout
